...
from ..serializers import RegisterSerializer, CustomAuthTokenSerializer, CustomTokenObtainPairSerializer, \
    ChangePasswordSerializer, ActivateResendSerializer
from rest_framework.response import Response
...
class RegisterApiViews(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            email_obj = EmailMessage('email/activation.tpl', {'token': token}, 'reza72rg@gmail.com', to= [email])
        ...
        try:
            ...
            if not user.is_verified:
                ...
                return Response({'email': 'Successfully activated'}, status=status.HTTP_200_OK)
                 ...
                 return Response({'error': 'Your accounts already been verified'}, status=status.HTTP_200_OK)                
...
class ActivationResendApiView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        token = self.get_tokens_for_user(user_object)
        email_obj = EmailMessage('email/activation.tpl', {'token': token}, 'reza72rg@gmail.com', to= [user_object.email])
        EmailThread(email_obj).start()
        return Response({'detail':'Email activate resend successfully'}, status=status.HTTP_200_OK)
...
'''class SendEmail(generics.GenericAPIView):
    ...
    def get(self,request, *args, **kwargs):
        ...
        token = self.get_tokens_for_user(user_object)
        email_obj = EmailMessage('email/activation.tpl', {'token': token}, 'reza72rg@gmail.com', to= [self.email])
        EmailThread(email_obj).start()
